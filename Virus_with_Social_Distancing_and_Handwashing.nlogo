...
A random population is set-up, with an initial infected percentage, over time the population moves, with the potential to infect others. A life cycle of healthy, infected, sick, immune is generall followed.
The Social Distancing parameters prevents people who adhere to it from moving to the same patch as others, which is the condition for transmission of a disease from an infected (NO symptoms showing) or sick person (symptoms showing).
